...
| `pantry_what_is_left` | O que sobrou na despensa depois dos pratos já aceitos. | nenhum | Estoque semeado menos consumo, com o histórico de qual prato levou quanto. É o que permite dizer "a lasanha levou 1 kg, sobraram 500 g" em vez de só "faltam 500 g". |
...
| Ferramenta | O que faz | Argumentos | Notas |
|---|---|---|---|
| `pricing_calculate_cmv` | Custeia uma porção, separando o que ela tem do que falta. | `dish`, `lines`, `portions`, `recipe_yields`, `researched_prices` | Devolve `open_questions` em vez de chutar quando a unidade da receita não bate com a da compra. Um ingrediente fora da despensa entra por `researched_prices` como **embalagem**, e a ferramenta divide a fração que a receita consome (vai para o CMV) das embalagens inteiras que ela precisa comprar (vão para a lista). Sem o preço pesquisado, o ingrediente volta em `not_found` e nada é precificado. Compara com o último CMV completo deste prato e devolve `cmv_changed_since_you_told_her` quando o número que ela já ouviu mudou. A primeira conta completa **fecha a receita do prato**: uma lista diferente depois disso é recusada, com a receita fechada devolvida. Devolve `cost_message_for_her`, a conta pronta em lista (um ingrediente por linha, ordenada pela que mais pesa, com a cauda dobrada em "e o resto (...)" e o total embaixo), e `cost_per_portion_for_her` sem a sigla. Com `recipe_yields`, as linhas são os totais da receita e a divisão pelo rendimento é feita aqui. |
...
| `pricing_price_scenarios` | Monta cenários de preço. | `cmv_per_portion`, `market` | Sem faixa de mercado devolve só o preço mínimo. Cada cenário projeta o lucro em doze meses com a inflação de alimentos. Devolve `options_for_her`, uma opção por linha, no mesmo formato de lista da conta. |
...
| `caveat_for_her` | A ressalva pronta, na língua dela, para ser dita **dentro** da frase. O badge de confiança não vem no resultado: é telemetria e fica no log |
| `message_pacing` | A forma do rascunho: em quantas partes está e onde é a costura |
